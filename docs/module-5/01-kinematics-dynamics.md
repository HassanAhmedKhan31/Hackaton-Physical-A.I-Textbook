...
```
*   **lower/upper:** The minimum and maximum angle (in radians) the joint can turn. This prevents the robot from snapping its own arm!
*   **effort:** The maximum torque (strength) the motor has. If this is too low, the robot's arm will simply fall under its own weight.
*   **velocity:** How fast the motor can spin.

Understanding these limits is the first step toward creating a robot that doesn't just move, but moves safely and effectively.